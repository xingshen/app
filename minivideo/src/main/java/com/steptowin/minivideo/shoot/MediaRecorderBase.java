...
import com.steptowin.minivideo.shoot.util.StringUtils;
...
//        Size size = null;
//        if (null != screenSize)
//            size = DeviceUtils.getOptimalPreviewSize(mParameters.getSupportedPreviewSizes(), screenSize.x, screenSize.y);

//        if (null != size)
//            mParameters.setPreviewSize(size.width, size.height);// 3:2
//        else
        List<Size> supportPreviewSize = mParameters.getSupportedPreviewSizes();
        if (null != supportPreviewSize) {
            for (Size size : supportPreviewSize) {
                Log.e("zhou", "support_w_h:" + size.width + "X" + size.height);
            }
        }

        mParameters.setPictureSize(480, 480);
